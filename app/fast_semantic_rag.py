...
class FastSemanticChunker:
    ...
    def _init_embedder(self):
        ...
        if _HAS_FASTEMBED:
            try:
                ...
                logger.info(
                    "✅ Successfully initialized FastEmbed for ultra-fast embeddings"
                )
                return
        ...
        if _HAS_SENTENCE_TRANSFORMERS:
            ...
            for model_name in models_to_try:
                try:
                    logger.info(
                        f"Attempting to initialize SentenceTransformer with {model_name}..."
                    )
                    ...
                    logger.info(
                        f"✅ Successfully initialized SentenceTransformer with {model_name}"
                    )
                    return

                except Exception as e:
                    logger.warning(
                        f"❌ SentenceTransformer with {model_name} failed: {e}"
                    )
                    continue
        ...
        logger.warning(
            "⚠️  Using dummy embeddings - all embedding methods failed. Install compatible versions of fastembed or sentence-transformers"
        )
    ...
    def semantic_chunk(
        self, text: str, chunk_size: int = 256, similarity_threshold: float = 0.7
    ) -> List[str]:
...
class FastEDAProcessor:
    ...
    def semantic_summary(self, texts: List[str]) -> str:
        ...
        summary_parts.append(
            f"- {eda_stats['avg_tokens_per_text']:.1f} average words per document"
        )
...
class FastVectorStore:
    ...
    def add_documents(
        self, texts: List[str], embeddings: np.ndarray, metadata: List[Dict]
    ):
...
class FastSemanticRAG:
    ...
    def __init__(
        self, config: FastRAGConfig = None, storage_path: str = "data/fast_rag"
    ):
    ...
    def _init_llm(self):
        ...
        logger.info(
            f"Attempting LLM initialization with fallback order: {fallback_order}"
        )
        ...
        for provider in fallback_order:
            ...
            try:
                llm = self._init_provider_llm(
                    provider, temperature, max_tokens, timeout
                )
                if llm:
    ...
    def _init_provider_llm(
        self, provider: str, temperature: float, max_tokens: int, timeout: int
    ):
    ...
    def add_documents(self, documents: List[str], metadata: List[Dict] = None):
        ...
        logger.info(
            f"Processed {len(documents)} documents ({len(all_chunks)} chunks) in {processing_time:.3f}s"
        )
    ...
    def query(self, question: str) -> Dict[str, Any]:
        ...
        try:
            ...
            return {
                "answer": final_answer
                or "I found relevant information but couldn't generate a complete response.",
    ...
    def _extract_pattern_insights(
        self, question: str, search_results: List[Tuple[str, Dict, float]]
    ) -> str:
        ...
        if any(word in question_lower for word in ["email", "contact", "reach"]):
            if all_emails:
                insights.append(
                    f"**📧 Email Addresses Found:**\n"
                    + "\n".join(f"• {email}" for email in sorted(all_emails))
                )

        if any(
            word in question_lower
            for word in ["skill", "technology", "tech", "programming"]
        ):
            if all_skills:
                ...
                insights.append(
                    f"**💻 Technical Skills:**\n"
                    + "\n".join(
                        f"• {skill}" for skill in list(skill_counts.keys())[:10]
                    )
                )
        ...
        if any(
            word in question_lower
            for word in ["eda", "analysis", "overview", "summary"]
        ):
...
{pattern_insights}"""
    ...
    def perform_eda(
        self, question: str = "Perform comprehensive EDA analysis"
    ) -> Dict[str, Any]:
...
def create_fast_semantic_rag(
    storage_path: str = "data/fast_rag", **config_kwargs
) -> FastSemanticRAG:
...
def create_advanced_rag_system(
    index_dir: str = "data/index", **kwargs
) -> FastSemanticRAG:
