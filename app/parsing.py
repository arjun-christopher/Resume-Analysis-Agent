...
def _perform_advanced_eda(text: str, entities: Dict[str, Any]) -> Dict[str, Any]:
    ...
    eda_results["text_statistics"] = {
        ...
        "lexical_diversity": len(set(word.lower() for word in words if word.isalpha()))
        / max(1, len(words)),
    }

    # Keyword density analysis
    word_freq = Counter(
        word.lower() for word in words if word.isalpha() and len(word) > 3
    )
    total_words = len(words)
    ...
    eda_results["experience_indicators"] = {
        ...
        "collaboration_score": sum(
            1 for kw in collaboration_keywords if kw in text_lower
        ),
    }
    ...
    for category, skills in SKILLS_DATABASE.items():
        found_skills = [
            skill for skill in skills if skill in entities.get("skills", [])
        ]
        if found_skills:
...
def extract_docs_to_chunks_and_records(
    paths: List[Path],
    ...
    for p in paths:
        ...
        for i, ch in enumerate(doc_chunks):
            ...
            if page2urls:
                ...
                page_guess = min(
                    total_pages,
                    max(1, round((i + 1) / max(1, len(doc_chunks)) * total_pages)),
                )
